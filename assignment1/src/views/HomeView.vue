<template>
    <section>
      ...
      <div class="mt-5">
        ...
        <div v-if="app.state.registrations.length===0" class="text-muted">No submissions yet.</div>
        <div v-else class="table-responsive">
          <table class="table align-middle">
            <thead><tr><th>Program</th><th>Name</th><th>Email</th><th>Age</th><th>Date</th><th>Submitted</th></tr></thead>
            <tbody>
              <tr v-for="r in app.state.registrations" :key="r.ts">
                <td>{{ r.program.title }}</td>
                <td>{{ r.form.name }}</td>
                <td>{{ r.form.email }}</td>
                <td>{{ r.form.age }}</td>
                <td>{{ r.form.prefDate }}</td>
  ...
  function submitForm(){
    if(!allValid()) return
    app.addRegistration({ program: selected.value, form: { ...form } })
    cancelApply()
